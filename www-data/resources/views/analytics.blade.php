...
@section('content')
    ...
    <form action="{{route('admin.dashboard.analytics')}}" method="get">
        ...
        <div class="col-12 col-md-4">
            <div class="form-group select-status">
                ...
                <select name="country" id="country" class="form-control select2">
                    ...
                    @foreach(Monarobase\CountryList\CountryListFacade::getList('en') as $key => $name)
                        <option value="{{$key}}" {{request()->input('country', 'all') == $key ? 'selected' : ''}}>{{$key}} - {{$name}}</option>
                    @endforeach
                </select>
                ...
                <div class="card dashboard-app">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <span class="t">Unique</span>
                                <span class="d">Result 1: {{$resultItem['unique_success_count']}} ({{$resultItem['launch_count'] === 0 ? 0 : round((($resultItem['unique_success_count'] / $resultItem['launch_count'])) * 100, 2)}}%)</span>
                                <span class="d">All: {{$resultItem['unique_count']}} ({{$resultItem['launch_count'] === 0 ? 0 : round((($resultItem['unique_count'] / $resultItem['launch_count'])) * 100, 2)}}%)</span>
                            </div>
                            <div class="col-6">
