...
class SxGeo
{
    ...
    public function get_num($ip)
    {
        ...
        if (0 == $ip1n || 10 == $ip1n || 127 == $ip1n || $ip1n >= $this->b_idx_len || false === ($ipn = ip2long($ip))) {
            return false;
        }
        $ipn = pack('N', $ipn);
        $this->ip1c = chr($ip1n);
    ...
    protected function readData($seek, $max, $type)
    {
        ...
        if ($seek && $max) {
            if ($this->memory_mode) {
                $raw = substr(1 == $type ? $this->regions_db : $this->cities_db, $seek, $max);
            } else {
                fseek($this->fh, $this->info[1 == $type ? 'regions_begin' : 'cities_begin'] + $seek);
    ...
    protected function unpack($pack, $item = '')
    {
        ...
        foreach ($pack as $p) {
            ...
            $type0 = $type[0];
            if ($empty) {
                $unpacked[$name] = 'b' == $type0 || 'c' == $type0 ? '' : 0;
                continue;
            }
            switch ($type0) {
            case 't':
