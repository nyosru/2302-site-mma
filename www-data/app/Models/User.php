...
class User extends Authenticatable
{
    ...
    {
        $userRole = Enforcer::getRolesForUser((string)$this->id);
    ...
    {
        return $this->hasOne(UserRoles::class, 'key', 'role_key');
    }

    public function createLog($model, $comment)
    {
        UserLog::create(
            [
                'user_id' => $this->id,
    ...
    {
        $url = route('admin.users.view', [$this]);
        return "<a href='{$url}'>User {$this->name} [{$this->id}]</a>";
    }
}
